...
module apump_ai::token_factory_tests {
    ...
    #[test(admin = @0x1)]
    fun test_buy_token(admin: &signer) {
        init_for_testing();
    ...
    #[test(admin = @0x1)]
    fun test_sell_token(admin: &signer) {
        init_for_testing();
    ...
    #[test(admin = @apump_ai)]
    fun test_admin_set_fee_percent(admin: &signer) {
        init_for_testing();
        let new_fee = 100;
    ...
    fun test_set_fee_percent_non_admin_fails(non_admin: &signer) {
        init_for_testing();
        token_factory::set_fee_percent(non_admin, 100);
        let fee_percent = token_factory::get_fee_percent(signer::address_of(non_admin));

        assert!(fee_percent == 100, 1);
    }

    #[test(admin = @apump_ai)]
    fun test_admin_set_required_collateral(admin: &signer) {
        init_for_testing();
    ...
    #[test(non_admin = @0x123)]
    #[expected_failure(abort_code = token_factory::ERR_NOT_ADMIN)]
    fun test_set_required_collateral_non_admin_fails(non_admin: &signer) {
        init_for_testing();
        token_factory::set_required_collateral(non_admin, 1000);
    }

    #[test(admin = @apump_ai)]
    fun test_admin_set_new_competition(admin: &signer) {
        init_for_testing();
    ...
    #[test(non_admin = @0x123)]
    #[expected_failure(abort_code = token_factory::ERR_NOT_ADMIN)]
    fun test_set_new_competition_non_admin_fails(non_admin: &signer) {
        init_for_testing();
    ...
    fun test_buy_insufficient_apt(admin: &signer, buyer: &signer) {
        init_for_testing();
        let buyer_addr = signer::address_of(buyer);
        let name = string::utf8(b"TestToken");
        let symbol = string::utf8(b"TT");
    ...
    #[expected_failure(abort_code = token_factory::ERR_APT_NOT_ENOUGH)]
    fun test_sell_insufficient_tokens(admin: &signer, seller: &signer) {
        init_for_testing();
        let seller_addr = signer::address_of(seller);
        let name = string::utf8(b"TestToken");
        let symbol = string::utf8(b"TT");
